...
public class GyroController : MonoBehaviour 
{
	...
	public float keyboardTipSpeed = 30.0f;
	...
	protected void Update() 
	{
		if (gyroEnabled)
		{
			...
			if( attitude == Quaternion.identity ){
				var horizInput = Input.GetAxis ( "Horizontal" );
				var vertInput = Input.GetAxis ( "Vertical" );
				
				attitude = Quaternion.Euler( - keyboardTipSpeed * vertInput, keyboardTipSpeed * horizInput, 0.0f);
			}

			transform.rotation = ConvertRotation(referanceRotation * attitude) * GetRotFix();
		} 
	}
